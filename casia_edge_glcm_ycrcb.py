...
def main():
    ...
    def prepare_data_from_filepaths(filepaths, transform):
        ...
        for fp in filepaths:
            try:
                ...
                # 3. Crチャネル（赤の色差成分）のみを抽出
                #    YCrCbは[Y, Cr, Cb]の順なので、インデックス1がCrチャネル
                cr_channel_np = ycrcb_np[:, :, 1]

                # 4. Scharrフィルタを "Crチャネル" に適用
                scharr_x = cv2.Scharr(cr_channel_np, cv2.CV_64F, dx=1, dy=0)
                scharr_y = cv2.Scharr(cr_channel_np, cv2.CV_64F, dx=0, dy=1)
    ...
    save_path = "output/casia_edge_glcm_prediction.png"
